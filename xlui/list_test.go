...
func testList() *ListLayer {
	...
	list.Items = []string{"a", "b", "c", "d", "e", "f", "g", "h", "i", "j"}
	list.Limit = 5
	return list
}
...
// TestListSelectItemOutOfRange ensures out of range selections are ignored.
func TestListSelectItemOutOfRange(t *testing.T) {
	list := testList()
	list.SelectItem(-1)
	if list.Selected != -1 {
		t.Errorf("SelectItem(-1): Selected = %d, want -1", list.Selected)
	}
	list.SelectItem(10)
	if list.Selected != -1 {
		t.Errorf("SelectItem(10): Selected = %d, want -1", list.Selected)
	}
}

// TestListSelectItemCallsOnSelect ensures OnSelect fires on selection.
func TestListSelectItemCallsOnSelect(t *testing.T) {
	list := testList()
	called := -1
	list.OnSelect = func(i int) { called = i }
	list.SelectItem(3)
	if called != 3 {
		t.Errorf("OnSelect = %d, want 3", called)
	}
	if list.Selected != 3 {
...
func TestListClickSelects(t *testing.T) {
	list := testList()
	res := list.click(xgal.Pt(50, 20), int(xgal.MouseButtonLeft))
	if res != Raise {
...
func TestListClickEmptyArea(t *testing.T) {
	list := testList()
	res := list.click(xgal.Pt(50, 98), int(xgal.MouseButtonLeft))
	if res != Raise {
		t.Errorf("click returned %v, want Raise", res)
	}
	if list.Selected != -1 {
...
func TestListClickRaisesAndDrags(t *testing.T) {
	...
	list := ui.List(xgal.Rect(0, 0, 100, 100))
	...
	if list.Selected != 0 {
		t.Fatalf("Selected = %d, want 0", list.Selected)
	}
